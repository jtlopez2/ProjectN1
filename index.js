...
router.get('/mongodb', function (request, response) {
           ...
           mongodb.MongoClient.connect('mongodb://heroku_117sqmxb:testpass1@ds159129.mlab.com:59129/heroku_117sqmxb', function(err, client) {
                                       // mongodb.MongoClient.connect(process.env.MONGODB_URI, function(err, db) {  // works with mongodb v2 but not v3
                                       if(err) throw err;
                                       //get collection of routes
                                       var db = client.db('heroku_117sqmxb');  // in v3 we need to get the db from the client
